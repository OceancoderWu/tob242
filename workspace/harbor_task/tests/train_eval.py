...
def main():
    ...
    p.add_argument('--data',type=Path,default=ASSETS/'data');p.add_argument('--test-data',type=Path,default=Path('/opt/benchmark/data'));p.add_argument('--smoke',action='store_true')
    ...
    for item in manifest['test_files']:
        if args.smoke and (item['k'],item['b']) not in [(2,1),(9,3)]:continue
        test=read_rows(args.test_data/item['file'])
        if args.smoke:test=test[:16]
    ...
    result=dict(status='smoke' if args.smoke else 'complete',seed=args.seed,best_epoch=epoch_best,best_val_accuracy=best,
                all24_macro_accuracy=sum(r['accuracy'] for r in tests)/len(tests),ood15_macro_accuracy=sum(r['accuracy'] for r in ood)/len(ood),
