...
def verify(train_data_dir,test_data_dir,manifest,include_tests=True):
...
def collate(rows,device,include_labels=True):
    ...
    labels=torch.tensor([r['label'] for r in rows],dtype=torch.long,device=device) if include_labels else None
    return graph,labels
