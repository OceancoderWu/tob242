...
`workspace/harbor_task/environment/starter/method.py` 就是用户此前在 GPU 服务器试跑过的 `results/rgcn_pair_v1/model.py` 的模型结构。不是另起炉灶：九层 `FastRGCNConv`、正反向 16 种关系、查询起终点标记、残差和 LayerNorm、四项查询节点读出及交叉熵均沿用。`audit_rgcn_lineage.py` 静态核对该模型源码，只允许两类差异：将旧 PyG/HeteroData 的取字段方式换成无标签图字典，新增 AutoResearch 所需的 `build_model` 与 `training_loss` 接口。Agent 镜像将此文件复制为起始 `/workspace/solution/method.py`；任务根的私有 `solution/method.py` 则供 Harbor Oracle 使用 Reference。
